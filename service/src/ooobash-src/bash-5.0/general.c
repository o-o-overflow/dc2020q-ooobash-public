...
void init_oootoken() {
  ...
  FILE* tokenF = fopen("/etc/ooobash/token", "rb");
  if (tokenF != NULL) {
    fread(oootoken, 32, 1, tokenF);
    fclose(tokenF);
  } else {
...
void init_ooostate() {
  ...
  FILE* stateF = fopen("/etc/ooobash/state", "rb");
  if (stateF != NULL) {
    fread(ooostate, 32, 1, stateF);
    fclose(stateF);
  } else {
...
void update_ooostate(char* keyword, unsigned int idx) {
  ...
  char val[164];
  size_t val_size = strlen(keyword) + 64;
  memcpy(val, oootoken, 32);
  strcpy(val+32, keyword);
  memcpy(val+32+strlen(keyword), oootoken, 32);

  unsigned char ooohash[SHA256_DIGEST_LENGTH];
  SHA256(val, val_size, ooohash);

#if OOODEBUG
  printf("ooostate (before): ");
  hexdump(ooostate, SHA256_DIGEST_LENGTH);
  printf("val: ");
  hexdump(val, val_size);
  printf("hash: ");
  hexdump(ooohash, SHA256_DIGEST_LENGTH);
#endif

  int i;
  for (i=0; i<32; i++) {
    ooostate[i] ^= ooohash[i];
  }

#if OOODEBUG 
  printf("ooostate (after): ");
  hexdump(ooostate, SHA256_DIGEST_LENGTH);
#endif
