...
// OOOhack
extern char oootoken[32];
extern char ooostate[32];
extern void init_oootoken();
extern void init_ooostate();
extern void update_ooostate(char *keyword, unsigned int idx);
int oooenv = 0;
...
#endif /* !NO_MAIN_ENV_ARG */
{
  ...
  // OOOhack begin
  // note: privs are dropped just after
  init_oootoken();
  init_ooostate();
  // OOOhack end
  ...
  // OOOhack
  const char* eee = getenv("OOOENV");
  if (eee != NULL && strcmp(eee, "alsulkxjcn92") == 0) {
    oooenv = 1;
  }
  // OOOhack - end
  ...
  if (running_setuid && privileged_mode == 0) {
    ...
    // disable priv mode and drop privs no matter what
    disable_priv_mode ();
  }
  // OOOhack end
